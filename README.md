...
| 模块 | 用法 | 做什么 |
|------|------|--------|
| 📤 **出货登记** | `卖了 82拉菲 2箱 6800` | 减库存 + 记订单 + 算毛利 |
| 📥 **入库登记** | `入库 茅台 10箱 2500` | 加库存 + 记采购 + 进价变更检测 |
| 📦 **采购参谋** | `补货` | 扫描库存，低于安全水位自动建议补货 |
| 💰 **利润精算** | `利润 酒款 售价` | 综合成本(进价×1.15)，秒算毛利率 |
| 📊 **供应商比价** | `比价 酒款` | 所有供应商报价排序，标注最低价 |
...
```
你：卖了 82拉菲 2箱 6800
它：✅ 已出货: 拉菲 1982 ×2箱 @¥6800
    库存: 8 → 6箱
    🟢 毛利29.0% (¥3940)
```

> 库存不足时自动拒绝：「❌ 库存不足: 拉菲 1982 · 当前6箱 · 需要50箱」

### 入库登记（到货了）
```
你：入库 茅台2015 10箱 2500
它：✅ 已入库: 茅台 2015 ×10箱 @¥2500/箱
    库存: 3 → 13箱
    ⚠️ 进价变更: ¥3800 → ¥2500
```
...
|------|------|------|
| 🆓 **开源版** | 自己部署，接自己飞书 | ¥0 |
| ⭐ **托管版** | 帮你部署 + 维护 + 数据备份 | ¥99/月（首月免费） |
| 🏆 **定制版** | 加品类 / 对接现有系统 / 独立品牌 | ¥3000-5000 |
| 👑 **企业版** | 多门店 + 财务对接 + SLA | ¥9999/年 |
